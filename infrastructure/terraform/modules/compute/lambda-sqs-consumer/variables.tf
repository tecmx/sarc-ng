...
variable "sqs_queue_arn" {
  description = "ARN of the SQS queue to consume messages from"
  type        = string
}
...
variable "function_response_types" {
  description = "List of current response type enums"
  type        = list(string)
  default     = []
}

variable "create_dlq" {
  description = "Whether to create a dead-letter queue for the event source mapping"
  type        = bool
  default     = false
}
...
variable "dlq_message_retention_seconds" {
  description = "Number of seconds that messages are kept in the dead-letter queue"
  type        = number
  default     = 1209600 # 14 days
}

variable "scaling_config" {
  description = "Configuration for scaling behavior of the event source mapping"
  type = object({
